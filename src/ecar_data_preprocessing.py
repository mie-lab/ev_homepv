...
def create_ecar_homes_table(home_table_info, DSN, df):
    ...
    with psycopg2.connect(**DSN) as conn:
        ...
        query= """SELECT AddGeometryColumn ('{schema_single}', '{home_table_name_single}', 'geometry', 4326, 'POINT',2);""".format(**home_table_info)
        cur.execute(query)
...
def fill_ecarid_is_athome_table(ecarid_athome_table_info, ecar_table_info,
    ...
    with psycopg2.connect(**DSN) as conn:
        ...
        query = sql.SQL("""drop table if exists temp_bmwid_ishome;
                ...
                AND {ecar_table_name}.{geom_field} is not null order by bmw_id;""").format(**{**home_table_info, **ecar_table_info, **start_end_dict})
        cur.execute(query)
...
def create_segmented_ecar_data(ecarid_athome_table_info, DSN):
    ...
    ecar_unique_timestamps = ecar_unique_timestamps.drop(ecar_unique_timestamps[ecar_unique_timestamps['vin']!= ecar_unique_timestamps['vin_lag']].index)
