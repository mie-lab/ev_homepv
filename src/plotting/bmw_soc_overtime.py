...
from methods.PV_interface import get_PV_generated
import numpy as np
import pandas as pd
from methods.helpers import soc2remainingCharge
...
# this one is shitty --> vin = "0052b13287adea1a991bc7acaf41710b"  '00000dcb1a3963f3ae1d91cd9755b2d0'
...
#pandas is not very reliable when loading timestamps...
...
all_bmwdata_df = all_bmwdata_df[(all_bmwdata_df['vin'] == vin) & ((all_bmwdata_df['timestamp_start_utc'] + pd.Timedelta(hours=24)) >= t_start)
...
charging_state_timeseries['timestamp'] = charging_state_timeseries['timestamp'].astype('datetime64[ns]') #transform it to timestamps again!
#set index and plot
charging_state_timeseries.set_index('timestamp', inplace=True)

#charging_state_timeseries['soc'] = charging_state_timeseries['soc'].apply(lambda x: soc2remainingCharge(1-x))
...
df2.sort_values(['timestamp','org_ix'],ascending=[1,1],inplace=True) #sort because the order affects the drawing of the polygones
...
ax.set_ylim((0, 101))
ax2.set_ylim((0, 10.1))
#ax.set_ylim((0, max(charging_state_timeseries['soc']) + 1))





# ax.set_title("SoC over time, real data, WoW")

# Shrink current axis's height by 10% on the bottom
# plt.subplots_adjust(bottom=0.25)
# Put a legend below current axis

lgd = ax2.legend(handles=[l1, pvgen_handle[0], l2, l3],
...
save_figure(figure_out, bbox_extra_artists=[lgd])
