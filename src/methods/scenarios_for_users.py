...
def scenario_2(data, user, max_charging_power):
    ...
    for i in range(len(user_data.index)):
        ...
        if i != 0:
            user_data.loc[ix, 'kWh_start'] = user_data['kWh_end'].iloc[
                i - 1]  # end has to be updated wiht the consumptions of the other segments
    ...
    for i in range(len(user_data.index)):
        ...
        if i > 0:
            battery_beginning = np.minimum(battery_capacity,
                                           user_data['battery_end_of_timestamp'][user_data.index[i - 1]] + \
                                           get_PV_generated(user_data['end'][user_data.index[i - 1]],
                                                            user_data['start'][user_data.index[i]], user,
                                                            path_to_data_folder))
...
def create_scenario_table(data_baseline, data_scenario_2, data, capacity, max_power_kw, path_to_data_folder):
    ...
    # print(user_list)
    print("baseline")
    scenario_baseline_list = [baseline(data_baseline, str(user_list[i]), max_power_kw, path_to_data_folder) for i in
                              range(len(user_list))]
    print("scenario 1")
    ...
    print("scenario 3")
    scenario_3_list = [scenario_3(data, str(user_list[i]), capacity, path_to_data_folder, max_power_kw) for i in
                       range(len(user_list))]

    list_of_tuples = list(
        zip(scenario_baseline_list, scenario_1_list, scenario_2_list, scenario_3_list, total_demand_list))
